...
values ('key', 'field', '', @expireAt)";
        ...
        [Fact, CleanDatabase]
        public void Execute_Optimizes_CounterTable()
        {
            using (var connection = CreateConnection())
            {
                // Arrange
                string createSql = @"INSERT INTO """ + GetSchemaName() + @""".""counter"" (""key"", ""value"") VALUES (@key, 1)";
                var entriesCount = 5;
                for (int i = 0; i < entriesCount; i++)
                {
                    connection.Execute(createSql, new { key = $"stats:succeeded:{i}", expireAt = DateTime.UtcNow.AddMonths(-1) });
                }
                for (int i = 0; i < entriesCount; i++)
                {
                    connection.Execute(createSql, new { key = $"stats:deleted:{i}", expireAt = DateTime.UtcNow.AddMonths(-1) });
                }
                ...
                // Assert
                var entryCount = connection.ExecuteScalar<long>(@"select count(*) from """ + GetSchemaName() + @""".""counter""");
                var succeededSum = connection.ExecuteScalar<long>(@"select sum(value) from """ + GetSchemaName() + @""".""counter"" where key='stats:succeeded'");
                var deletedSum = connection.ExecuteScalar<long>(@"select sum(value) from """ + GetSchemaName() + @""".""counter"" where key='stats:deleted'");

                Assert.Equal(2, entryCount); // stats:succeeded | stats:deleted
                Assert.Equal(entriesCount, succeededSum);
                Assert.Equal(entriesCount, deletedSum);
            }
        }
...
insert into """ + GetSchemaName() + @""".""list""(""key"", ""value"", ""expireat"")
...
values ('key', 1, now() at time zone 'utc' - interval '{0} seconds') returning ""id""";
        ...
        private static bool IsEntryExpired(NpgsqlConnection connection, int entryId)
        {
            var count = connection.Query<long>(
                @"select count(*) from """ + GetSchemaName() + @""".""list"" where ""id"" = @id", new { id = entryId }).Single();
            return count == 0;
        }
